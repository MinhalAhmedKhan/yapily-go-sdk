...
```go
yapily.WithServerURL("https://api.example.com")
```
...
```go
yapily.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
yapily.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
yapily.WithTimeout(30 * time.Second)
```
